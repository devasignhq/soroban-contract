...
fn test_refund_success_no_contributor() {
    ...
    let (task_id, bounty_amount) = setup_task_for_refund(
        &env,
        &client,
        &usdc_token,
        &creator,
...
fn test_refund_success_with_contributor() {
    ...
    let creator = Address::generate(&env);
    let (task_id, _) = setup_task_for_refund(
        &env,
        &client,
        &usdc_token,
        &creator,
...
fn test_refund_unauthorized() {
    ...
    let creator = Address::generate(&env);
    let (task_id, _) =
        setup_task_for_refund(&env, &client, &usdc_token, &creator, "refund_unauth", false);
...
fn test_refund_invalid_task_id() {
    ...
    // Test empty task ID
    let result = client.try_refund(&String::from_str(&env, &"".to_string()));
    assert!(result.is_err());
    assert_eq!(result.unwrap_err().unwrap(), Error::EmptyTaskId);

    // Test too short task ID
    let result = client.try_refund(&String::from_str(&env, &"ab".to_string()));
    assert!(result.is_err());
    assert_eq!(result.unwrap_err().unwrap(), Error::InvalidTaskId);

    // Test too long task ID
    let long_id = "a".repeat(101);
    let result = client.try_refund(&String::from_str(&env, &long_id));
    assert!(result.is_err());
    assert_eq!(result.unwrap_err().unwrap(), Error::InvalidTaskId);
}
