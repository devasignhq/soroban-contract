...
use devasign_task_escrow::{TaskStatus, DisputeResolution};
...
fn test_complete_dispute_workflow_partial_payment() {
    ...
    let result = client.resolve_dispute(&task_id, &DisputeResolution::PartialPayment(contributor_amount));
    assert!(result == ());
    ...
    assert_eq!(final_contributor_balance, initial_contributor_balance + contributor_amount);
    assert_eq!(final_creator_balance, initial_creator_balance + creator_refund);
}
...
fn test_mixed_workflow_scenarios() {
    ...
    usdc_token.mint(&creator, &bounty_amount);
    client.create_escrow(&creator, &task_id_1, &bounty_amount);
    ...
    usdc_token.mint(&creator, &bounty_amount);
    client.create_escrow(&creator, &task_id_2, &bounty_amount);
    client.refund(&task_id_2);
    
    // Scenario 3: Dispute resolution
    let task_id_3 = TestValidation::generate_task_id(&env, "mixed_dispute", 3);
    usdc_token.mint(&creator, &bounty_amount);
    client.create_escrow(&creator, &task_id_3, &bounty_amount);
    ...
    assert_eq!(escrow_1.status, TaskStatus::Resolved);
    assert_eq!(escrow_2.status, TaskStatus::Cancelled);
    assert_eq!(escrow_3.status, TaskStatus::Resolved);
...
fn test_admin_operations_workflow() {
    ...
    client.resolve_dispute(&task_id, &DisputeResolution::PartialPayment(bounty_amount / 2));
