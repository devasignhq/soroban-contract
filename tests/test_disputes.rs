...
use devasign_task_escrow::{TaskEscrowContractClient, Error, TaskStatus, DisputeResolution};
...
fn test_dispute_task_by_creator() {
    ...
    let (task_id, _) = setup_task_for_dispute(&env, &client, &usdc_token, &creator, &contributor, "creator_dispute", true);
...
fn test_dispute_task_by_contributor() {
    ...
    let creator = Address::generate(&env);
    let contributor = Address::generate(&env);
    let (task_id, _) = setup_task_for_dispute(&env, &client, &usdc_token, &creator, &contributor, "contrib_dispute", true);
...
fn test_dispute_task_unauthorized() {
    ...
    let (task_id, _) = setup_task_for_dispute(&env, &client, &usdc_token, &creator, &contributor, "unauth_dispute", true);
...
fn test_dispute_task_invalid_reason() {
    ...
    let creator = Address::generate(&env);
    let contributor = Address::generate(&env);
    let (task_id, _) = setup_task_for_dispute(&env, &client, &usdc_token, &creator, &contributor, "invalid_reason", true);
...
fn test_resolve_dispute_pay_contributor() {
    ...
    let creator = Address::generate(&env);
    let contributor = Address::generate(&env);
    let (task_id, bounty_amount) = setup_task_for_dispute(&env, &client, &usdc_token, &creator, &contributor, "resolve_pay", true);
...
fn test_resolve_dispute_refund_creator() {
    ...
    let creator = Address::generate(&env);
    let contributor = Address::generate(&env);
    let (task_id, bounty_amount) = setup_task_for_dispute(&env, &client, &usdc_token, &creator, &contributor, "resolve_refund", true);
    ...
    assert_eq!(final_creator_balance, initial_creator_balance + bounty_amount);
...
fn test_resolve_dispute_partial_payment() {
    ...
    let creator = Address::generate(&env);
    let contributor = Address::generate(&env);
    let (task_id, bounty_amount) = setup_task_for_dispute(&env, &client, &usdc_token, &creator, &contributor, "resolve_partial", true);
    ...
    let result = client.resolve_dispute(&task_id, &DisputeResolution::PartialPayment(partial_amount));
    assert!(result == ());
    ...
    assert_eq!(final_contributor_balance, initial_contributor_balance + partial_amount);
    assert_eq!(final_creator_balance, initial_creator_balance + refund_amount);
...
fn test_resolve_dispute_unauthorized() {
    ...
    let creator = Address::generate(&env);
    let contributor = Address::generate(&env);
    let (task_id, _) = setup_task_for_dispute(&env, &client, &usdc_token, &creator, &contributor, "unauth_resolve", true);
...
fn test_resolve_dispute_not_disputed() {
    ...
    let creator = Address::generate(&env);
    let contributor = Address::generate(&env);
    let (task_id, _) = setup_task_for_dispute(&env, &client, &usdc_token, &creator, &contributor, "not_disputed", true);
...
fn test_get_dispute_info_not_disputed() {
    ...
    let creator = Address::generate(&env);
    let contributor = Address::generate(&env);
    let (task_id, _) = setup_task_for_dispute(&env, &client, &usdc_token, &creator, &contributor, "no_dispute", true);
...
fn test_dispute_resolution_scenarios() {
    ...
    for (i, (_, resolution)) in resolutions.iter().enumerate() {
        let creator = Address::generate(&env);
        let contributor = Address::generate(&env);
        let (task_id, bounty_amount) = setup_task_for_dispute(&env, &client, &usdc_token, &creator, &contributor, &format!("scenario_{}", i), true);
