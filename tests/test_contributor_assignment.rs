...
fn test_assign_contributor_unauthorized() {
    ...
    let result = client.mock_auths(&[]).try_assign_contributor(&task_id, &contributor);
...
fn test_assign_contributor_invalid_task_id() {
    ...
    // Test empty task ID
    let result = client.try_assign_contributor(
...
fn test_assign_multiple_contributors_different_tasks() {
    ...
    for (i, contributor) in contributors.iter().enumerate() {
        let task_id = setup_escrow_for_assignment(&env, &client, &usdc_token, &creator, &format!("multi{}", i));
...
fn test_assign_same_contributor_multiple_tasks() {
    ...
    for i in 0..3 {
        let task_id = setup_escrow_for_assignment(&env, &client, &usdc_token, &creator, &format!("same{}", i));
