...
fn test_approve_completion_success() {
    ...
    let (task_id, bounty_amount) = setup_completed_task(&env, &client, &usdc_token, &creator, &contributor, "approve");
...
fn test_approve_completion_unauthorized() {
    ...
    let creator = Address::generate(&env);
    let contributor = Address::generate(&env);
    let (task_id, _) = setup_completed_task(&env, &client, &usdc_token, &creator, &contributor, "unauth");
...
#[test]
fn test_approve_completion_task_not_completed() {
    let (
        env, 
        admin, 
        usdc_address,
    ...
    // Create escrow and assign contributor but don't complete
    usdc_token.mint(&creator, &bounty_amount);
    client.create_escrow(&creator, &task_id, &bounty_amount);
    client.assign_contributor(&task_id, &contributor);
...
fn test_approve_completion_already_resolved() {
    ...
    let creator = Address::generate(&env);
    let contributor = Address::generate(&env);
    let (task_id, _) = setup_completed_task(&env, &client, &usdc_token, &creator, &contributor, "resolved");
...
fn test_approve_completion_invalid_task_id() {
    ...
    // Test empty task ID
    let result = client.try_approve_completion(
...
fn test_get_contract_usdc_balance() {
    ...
    let bounty_amount = TestConfig::MEDIUM_AMOUNT;
    usdc_token.mint(&creator, &bounty_amount);
    client.create_escrow(&creator, &TestValidation::generate_task_id(&env, "balance", 1), &bounty_amount);
